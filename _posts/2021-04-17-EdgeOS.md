...
{% highlight bash %}
sudo su
echo "service dnscrypt-proxy restart" > /config/scripts/dnscrypt-proxy.sh
chmod +x /config/scripts/dnscrypt-proxy.sh
configure
set system task-scheduler task restartDnscryptProxy executable path /config/scripts/dnscrypt-proxy.sh
set system task-scheduler task restartDnscryptProxy crontab-spec "0 2 * * *"
commit;save;exit
{% endhighlight %}
